---
title: 追蹤和分析AI流量
description: 瞭解如何在Adobe Customer Journey Analytics中識別和篩選AI產生的流量。
feature: Derived Fields
topic: Artificial Intelligence
role: Developer, User
level: Intermediate
doc-type: Feature Video
duration: 230
last-substantial-update: 2026-01-07T00:00:00.000Z
jira: KT-20054
exl-id: 2cd7693b-7534-408f-a128-b782222a4762
TQID: https://experienceleague.adobe.com/fu-KhUzNOSws548gvOAWFQ6Jb7YZke5HQSfWOlmzP-Q
product_v2:
  - id: e98b7246-966c-4318-9e95-cad2f7a17dc7
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2:
  - id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
  - id: bbbea26f-9621-49eb-9ab8-e06fb3bbce8c
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: fb8bcbdd846b74e46321c69b4ccee3752cbea5d4
workflow-type: tm+mt
source-wordcount: 100
ht-degree: 41%

---

# 追蹤和分析 AI 流量

了解如何運用衍生的欄位、區段和 Workspace 專案，在 Adobe Customer Journey Analytics 中識別及篩選 AI 產生的流量，以確保獲得準確、以人為中心的客戶洞察。 此針對管理員和行銷人員的快速教學課程會顯示透過使用者代理程式、反向連結等檢測LLM機器人的逐步設定，讓您的資料保持乾淨和可操作。

如需詳細資訊，請審閱 Content Analytics [文件](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-usecases/derived-fields/ai-traffic){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/3478586/?captions=chi_hant&learn=on&enablevpops)
