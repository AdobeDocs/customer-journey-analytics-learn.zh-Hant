---
title: 將文字視覺效果新增至Analysis Workspace專案
description: 瞭解如何在Customer Journey Analytics的Analysis Workspace專案中新增及設定文字視覺效果。
feature: Visualizations
doc-type: technical video
team: Technical Marketing
kt: 13426
role: User
level: Beginner
last-substantial-update: 2026-02-09T00:00:00Z
exl-id: f4ddd3fb-af0b-4594-9cde-81829885b611
source-git-commit: 0e2ff3688878b5d2e8f3373350608c040333225f
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 0%

---

# 將[!UICONTROL 文字]視覺效果新增至[!DNL Analysis Workspace]中的[!DNL Customer Journey Analytics]個專案

瞭解如何在Customer Journey Analytics的Analysis Workspace專案中新增及設定文字視覺效果。 如果您想要傳達關於量度的疑慮，或想要將內容新增至專案中的其他資料，此視覺效果相當實用。

>[!VIDEO](https://video.tv.adobe.com/v/3479695/?quality=12&learn=on)

如需[!UICONTROL 文字]視覺效果的詳細資訊，請瀏覽[檔案](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/text.html){target="_blank"}。
