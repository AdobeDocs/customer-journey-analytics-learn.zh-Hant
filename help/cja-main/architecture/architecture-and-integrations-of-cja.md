---
title: Customer Journey Analytics 的架構和整合
description: 瞭解Adobe Customer Journey Analytics如何在Adobe Experience Platform上建構以支援歷程和跨管道分析。
feature: Workspace Basics
team: Technical Marketing
thumbnail: 32483.jpg
kt: 4449
topic: Architecture
role: Developer
level: Intermediate
duration: 360
last-substantial-update: 2026-04-20T00:00:00Z
exl-id: 441e632f-b8d0-428a-9db6-c048bf173443
source-git-commit: e0ca48dbccbe942236c9d50b9ebce5fe64a093d7
workflow-type: tm+mt
source-wordcount: '104'
ht-degree: 25%

---

# Customer Journey Analytics的架構和整合

瞭解Adobe Customer Journey Analytics如何在Adobe Experience Platform上建構以支援歷程和跨管道分析。 此影片說明連線、資料檢視和資料彙整等重要的CJA概念，加上CJA如何與Real-Time CDP、Journey Optimizer和BI工具整合，以進行端對端客戶歷程深入分析。

如需 Customer Journey Analytics 的相關詳細資訊，請前往[文件](https://experienceleague.adobe.com/tw/en/docs/analytics-platform/using/cja-landing){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/32483/?learn=on&quality=12)
